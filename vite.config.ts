...
const apiTargets: Record<string, string> = {
  danbooru: 'https://danbooru.donmai.us', gelbooru: 'https://gelbooru.com',
  safebooru: 'https://safebooru.org', yandere: 'https://yande.re', rule34: 'https://api.rule34.xxx',
};
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (request, response, next) => {
        ...
        if (!target) { response.statusCode = 403; response.end('Unknown API source'); return; }
        try {
          const chunks: Buffer[] = [];
          ...
          const upstream = await fetch(`${target}${match[2]}`, { method: request.method, headers, body, redirect: 'follow' });
          response.statusCode = upstream.status;
