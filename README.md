...
Danbooru Viewer contains no analytics or telemetry. Preferences and source credentials are stored on the device in browser extension storage; credentials are not additionally encrypted. Favorites, download history, and cached media remain in IndexedDB on the local device. Credentials are sent only to the corresponding Booru API when that source is used or tested.
...
项目不包含分析或遥测。偏好设置和图源凭据保存在本机浏览器扩展存储中，凭据未额外加密；收藏、下载历史和媒体缓存保存在本机 IndexedDB。凭据仅在使用或测试对应图源时发送到该图源 API。
