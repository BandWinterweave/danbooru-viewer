...
export const useFavoriteStore = create<FavoriteStore>((setState, getState) => ({
  ...
  toggleRemote: async (post) => { const adapter = getBooruAdapter(post.source); const credentials = credentialsFor(post); if (!credentials?.username || !credentials.apiKey || !adapter.addFavorite) throw new Error('This source needs API credentials or does not support remote favorites'); const favorited = getState().isRemote(post); if (favorited) { if (!adapter.removeFavorite) throw new Error('This source cannot remove remote favorites'); await adapter.removeFavorite(post.id, credentials); } else await adapter.addFavorite(post.id, credentials); setState((state) => ({ remoteOverrides: { ...state.remoteOverrides, [favoriteKey(post)]: !favorited } })); },
  ...
  importJson: async (file) => { const data = JSON.parse(await file.text()) as { favorites?: UnifiedPost[]; groups?: FavoriteGroup[] }; if (!Array.isArray(data.favorites) || !Array.isArray(data.groups)) throw new Error('Invalid favorites file'); await Promise.all([del(FAVORITES_KEY), del(GROUPS_KEY)]); await persist(data.favorites, data.groups); setState({ favorites: data.favorites, groups: data.groups }); },
}));
