import type { ApiProxyFailureReason, ApiProxyResponse } from '../types/api';
...
function invalid(status: number, error: string, reason: ApiProxyFailureReason = 'invalid'): ApiProxyResponse { return { ok: false, status, reason, error }; }
...
function validateMessage(message: unknown) {
  ...
  if (!isRecord(message) || message.type !== 'API_REQUEST' || !isRecord(message.payload)) return invalid(400, network.invalidPayload);
  const { requestId, url: rawUrl, method: rawMethod = 'GET', headers: rawHeaders = {}, body } = message.payload;
  if (requestId !== undefined && (typeof requestId !== 'string' || requestId.length > 128)) return invalid(400, network.invalidRequestId);
  if (typeof rawUrl !== 'string' || rawUrl.length > 8192) return invalid(400, network.invalidUrl);
  if (rawMethod !== 'GET' && rawMethod !== 'POST' && rawMethod !== 'DELETE') return invalid(405, network.methodNotAllowed);
  if (!isRecord(rawHeaders)) return invalid(400, network.invalidHeaders);
  let url: URL;
  try {
    url = new URL(rawUrl);
  } catch {
    return invalid(400, network.invalidUrl);
  }
  if (!ALLOWED_ORIGINS.has(url.origin) || url.protocol !== 'https:' || url.username || url.password) return invalid(403, network.hostNotAllowed);
  const methods = url.origin === 'https://danbooru.donmai.us' ? ['GET', 'POST', 'DELETE'] : url.origin === 'https://gelbooru.com' ? ['GET', 'POST'] : ['GET'];
  if (!methods.includes(rawMethod)) return invalid(405, network.methodNotAllowed);
  const headers: Record<string, string> = {};
  for (const [name, value] of Object.entries(rawHeaders)) {
    const normalized = name.toLowerCase();
    if (!ALLOWED_HEADERS.has(normalized) || typeof value !== 'string' || value.length > 4096 || /[\r\n]/.test(value)) return invalid(400, network.invalidHeaders);
    headers[normalized === 'authorization' ? 'Authorization' : normalized === 'content-type' ? 'Content-Type' : 'Accept'] = value;
  }
  if (headers.Authorization && (url.origin !== 'https://danbooru.donmai.us' || !/^Basic [A-Za-z0-9+/]+=*$/.test(headers.Authorization))) return invalid(400, network.invalidAuthorization);
  if (body !== undefined && (typeof body !== 'string' || body.length > MAX_BODY_LENGTH || rawMethod !== 'POST')) return invalid(400, network.invalidBody);
  if (body && !['application/json', 'application/x-www-form-urlencoded'].some((type) => headers['Content-Type']?.toLowerCase().startsWith(type))) return invalid(400, network.invalidContentType);
...
async function performFetch(url: URL, method: string, headers: Record<string, string>, body: string | undefined, cacheable: boolean, cacheKey: string, signal: AbortSignal): Promise<ApiProxyResponse> {
  ...
  try {
    ...
    if (response.redirected && response.url && new URL(response.url).origin !== url.origin) return invalid(403, network.redirectNotAllowed, 'invalid');
    ...
    if (htmlResponse) {
      ...
      return invalid(response.status, cloudflare ? network.danbooruChallenge : network.unexpectedHtml(url.hostname), 'invalid');
    }
    ...
    return invalid(0, aborted ? signal.reason === 'cancelled' ? network.requestCancelled : network.requestTimedOut : network.networkFailed, aborted ? signal.reason === 'cancelled' ? 'cancelled' : 'timeout' : 'network');
  }
}
...
function finalizeResponse(url: URL, method: string, cacheable: boolean, cacheKey: string, status: number, statusText: string, contentType: string, responseText: string): ApiProxyResponse {
  ...
  if (htmlResponse) {
    const cloudflare = url.origin === 'https://danbooru.donmai.us' && /Just a moment|challenges\.cloudflare\.com|cf-chl-/i.test(responseText);
    return invalid(status, cloudflare ? network.danbooruChallenge : network.unexpectedHtml(url.hostname), 'invalid');
  }
  if (status < 200 || status >= 300) return invalid(status, network.requestFailed(status, ''), 'http');
  try {
    ...
    return invalid(status, network.unexpectedHtml(url.hostname), 'invalid');
  }
}

function abortError() { return new DOMException(getMessages().domainActions.network.requestTimedOut, 'AbortError'); }
