...
export const en = {
  ...
  comfy: {
    ...
    historyCompleted: 'Success', historyCancelled: 'Cancelled', historyFailed: (reason?: string) => reason ? `Failed (${reason})` : 'Failed',
    ...
    settingsDescription: 'Local workflow execution, task history, media cache, and optional sending from other websites. Only 127.0.0.1 is accepted.', serverAddress: 'Server address', historyRecords: 'History records', storageLimit: 'Storage limit (MB)', replaceReverse: 'Replace REVERSE text with formatted image tags', cacheOutputs: 'Cache output images locally', pageIntegration: 'Enable Send to ComfyUI on other websites', pageIntegrationDescription: 'Requests optional access to ordinary HTTP and HTTPS pages. Hovered large images can be queued, and the D launcher opens the full workbench.', pageImageMinPixels: 'Minimum image pixels', pagePermissionDenied: 'Website access was not granted. The feature remains off.', saveSettings: 'Save ComfyUI settings', saved: 'Saved',
  },
  shell: shellMessages,
  posts: postMessages,
