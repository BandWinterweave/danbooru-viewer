...
export function FavoriteGroups() {
  ...
  return <div className="sidebar-section favorite-groups"><h2>{actionMessages.favorites.title} <span>{favorites.length}</span></h2>
    <button onClick={() => { const name = window.prompt(actionMessages.favorites.groupNamePrompt); if (name) void createGroup(name); }}>{actionMessages.favorites.createGroup}</button>
    {groups.map((group) => {
      ...
      return <div className="favorite-group-block" key={group.id}>
        <div className="favorite-group"><button title={actionMessages.favorites.toggleGroup(open, group.name)} onClick={() => setExpanded(open ? null : group.id)}>{open ? <ChevronDown size={12} /> : <ChevronRight size={12} />}</button><Heart size={12} />{group.name}<span>{posts.length}</span><button disabled={!posts.length} title={actionMessages.favorites.downloadGroup(group.name)} onClick={() => void downloadPosts(posts)}><Download size={11} /></button><button title={actionMessages.favorites.deleteGroup(group.name)} onClick={() => void deleteGroup(group.id)}><Trash2 size={11} /></button></div>
        {open && <div className="group-posts">{posts.length ? posts.map((post) => <div key={favoriteKey(post)}><button title={actionMessages.favorites.openPost(post.id)} onClick={() => openDetail(post)}><CachedImage src={displayImageUrl(post.previewUrl)} alt="" /></button><button title={actionMessages.favorites.removePostFromGroup(post.id, group.name)} onClick={() => void toggleInGroup(group.id, post)}><Trash2 size={10} /></button></div>) : <p>{actionMessages.favorites.emptyGroup}</p>}</div>}
      </div>;
    })}
  </div>;
}
