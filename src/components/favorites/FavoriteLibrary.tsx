import { ArrowDown, ArrowUp, Check, Download, Film, FolderPlus, Import, Pencil, Search, Sparkles, Trash2, X } from 'lucide-react';
...
export function FavoriteLibrary() {
  ...
  return <section className="favorite-library-view" aria-labelledby="favorite-library-title">
    ...
    <div className="favorite-batch-bar"><button onClick={() => setSelected(visible.map(favoriteKey))}><Check size={14} />{messages.selectVisible}</button>{selectedGroup && <button title="Send every item in this group" onClick={() => void sendPostsToComfy(favorites.filter((post) => selectedGroup.postKeys.includes(favoriteKey(post))), selectedGroup.id)}><Sparkles size={14} />ComfyUI group</button>}{selected.length > 0 && <><strong>{messages.selectedCount(selected.length)}</strong><button onClick={() => void sendPostsToComfy(favorites.filter((post) => selectedSet.has(favoriteKey(post))))}><Sparkles size={14} />ComfyUI</button><select aria-label={messages.chooseGroup} value={targetGroup} onChange={(event) => setTargetGroup(event.target.value)}><option value="">{messages.chooseGroup}</option>{groups.map((group) => <option value={group.id} key={group.id}>{group.name}</option>)}</select><button disabled={!targetGroup} onClick={() => runStorage(store.addManyToGroup(targetGroup, selected))}><FolderPlus size={14} />{messages.addToGroup}</button><button disabled={!selectedGroup} onClick={() => { if (selectedGroup && window.confirm(messages.confirmRemoveFromGroup(selected.length, selectedGroup.name))) runStorage(store.removeManyFromGroup(selectedGroup.id, selected)); }}><X size={14} />{messages.removeFromGroup}</button><button className="is-destructive" onClick={() => { if (window.confirm(messages.confirmRemoveFavorites(selected.length))) runStorage(store.removeManyFavorites(selected)); }}><Trash2 size={14} />{messages.removeFavorites}</button><button className="icon-button" title={messages.clearSelection} onClick={() => setSelected([])}><X size={14} /></button></>}</div>
    {visible.length ? <div className="favorite-result-grid">{visible.map((post) => { const key = favoriteKey(post); const checked = selectedSet.has(key); return <article key={key} className={checked ? 'is-selected' : ''} onMouseEnter={() => setHoveredPost(post)} onMouseMove={() => setHoveredPost(post)} onMouseLeave={() => clearHoveredPost(post)}><button className="favorite-select" aria-label={messages.selectPost(post.id)} aria-pressed={checked} onClick={() => setSelected((current) => current.includes(key) ? current.filter((item) => item !== key) : [...current, key])}><Check size={14} /></button><button className="favorite-open" title={messages.openPost(post.id)} onClick={() => openDetail(post, 'favorites')}><MediaPreview post={post} />{isVideoPost(post) && <span className="media-kind"><Film size={11} /> {post.fileExt.toUpperCase()}</span>}</button><footer><span>{post.source} #{post.id}</span><strong>{post.score}</strong></footer></article>; })}</div> : <p className="favorite-library-empty">{messages.emptyLibrary}</p>}
