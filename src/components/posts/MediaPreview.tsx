...
export function MediaPreview({ post, className = '', eager = false }: { post: UnifiedPost; className?: string; eager?: boolean }) {
  ...
  if (isVideoPost(post)) return <video className={className} src={displayImageUrl(post.playbackUrl || post.fileUrl)} poster={imageUrl ? displayImageUrl(imageUrl) : undefined} muted loop autoPlay={eager} playsInline preload={eager ? 'auto' : 'none'} onMouseEnter={(event) => { if (!eager) void event.currentTarget.play().catch(() => undefined); }} onMouseLeave={(event) => { if (!eager) event.currentTarget.pause(); }} onError={() => setFailed(true)} />;
