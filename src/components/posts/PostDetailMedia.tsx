...
export function PostDetailMedia({ post, quality }: { post: UnifiedPost; quality: DetailImageQuality }) {
  ...
  useEffect(() => {
    setScale(1); setOffset({ x: 0, y: 0 }); setFailed(false); setLoaded(false);
  ...
  const startDrag = (event: React.PointerEvent) => {
    if (scale <= 1) return;
    dragging.current = true; lastPoint.current = { x: event.clientX, y: event.clientY }; event.currentTarget.setPointerCapture(event.pointerId);
  };
  ...
  return <div className="detail-media-zoom" onWheel={zoom} onDoubleClick={reset} onPointerDown={startDrag} onPointerMove={drag} onPointerUp={endDrag} onPointerCancel={endDrag}>
