...
export function ImageViewer() {
  ...
  const slides = viewable.map((post) => ({ src: displayImageUrl(displayMediaUrl(post)), alt: `${post.source} post ${post.id}`, width: post.imageWidth, height: post.imageHeight }));
  ...
  return <Lightbox open={open} close={close} index={viewerIndex} slides={slides} plugins={[Zoom, Fullscreen, Slideshow]} zoom={{ ref: zoomRef, scrollToZoom: false, zoomInMultiplier: 1.25, maxZoomPixelRatio: 6 }} slideshow={{ ref: slideshowRef, delay: interval * 1000 }} on={{ view: ({ index }) => { setViewerIndex(index); setZoomLevel(1); }, zoom: ({ zoom }) => setZoomLevel(zoom) }} render={{ slide: ({ slide, offset }) => { const post = viewable.find((item) => displayImageUrl(displayMediaUrl(item)) === slide.src); return post && isVideoPost(post) ? <video className="viewer-video" src={displayImageUrl(post.playbackUrl || post.fileUrl)} controls autoPlay={offset === 0} preload={offset === 0 ? 'auto' : 'metadata'} loop playsInline /> : undefined; }, slideContainer: ({ slide, children }) => { const post = viewable.find((item) => displayImageUrl(displayMediaUrl(item)) === slide.src); return <div className="viewer-slide" onWheel={(event) => { const zoom = zoomRef.current; if (!zoom || zoom.disabled) return; const target = Math.min(zoom.maxZoom, Math.max(zoom.minZoom, zoom.zoom * Math.exp(-event.deltaY * .0015))); zoom.changeZoom(target, true); }}>{children}{post && post.id === activePost?.id && !isVideoPost(post) && <NoteOverlay post={post} notes={notes} />}</div>; }, controls: () => activePost ? <div className="viewer-download"><DownloadMenu post={activePost} compact /><label title="Zoom level">{Math.round(zoomLevel * 100)}%</label><label title="Slideshow interval">{interval}s</label></div> : null }} carousel={{ finite: false }} />;
}
