...
import { activatePageIntegration, removePageIntegrationPermission, requestPageIntegrationPermission } from '../../services/comfy/page-integration';
...
export function SettingsPanel() {
  ...
  const [pagePermissionDenied, setPagePermissionDenied] = useState(false);
  ...
  useEffect(() => {
    if (!settings.comfyPageIntegrationEnabled || typeof chrome === 'undefined' || !chrome.permissions?.contains) return;
    let active = true;
    void chrome.permissions.contains({ origins: ['<all_urls>'] }).then((granted) => { if (active && !granted) settings.setComfyPageIntegrationEnabled(false); }).catch(() => undefined);
    const removed = (permissions: chrome.permissions.Permissions) => { if (permissions.origins?.includes('<all_urls>')) settings.setComfyPageIntegrationEnabled(false); };
    const removedEvent = chrome.permissions.onRemoved as unknown as { addListener: (listener: typeof removed) => void; removeListener?: (listener: typeof removed) => void };
    removedEvent.addListener(removed);
    return () => { active = false; removedEvent.removeListener?.(removed); };
  }, [settings.comfyPageIntegrationEnabled, settings.setComfyPageIntegrationEnabled]);
  ...
  const togglePageIntegration = async (enabled: boolean) => {
    setPagePermissionDenied(false);
    if (!enabled) {
      try {
        await removePageIntegrationPermission();
        settings.setComfyPageIntegrationEnabled(false);
      } catch {
        setPagePermissionDenied(true);
      }
      return;
    }
    if (!await requestPageIntegrationPermission()) {
      setPagePermissionDenied(true);
      return;
    }
    settings.setComfyPageIntegrationEnabled(true);
    try {
      await activatePageIntegration(settings.comfyPageImageMinPixels);
    } catch {
      settings.setComfyPageIntegrationEnabled(false);
      await chrome.permissions.remove({ origins: ['<all_urls>'] }).catch(() => undefined);
      setPagePermissionDenied(true);
    }
  };
  ...
  return (
    <main className="settings-page">
      ...
      <section className="settings-section"><div><h2>{comfyMessages.name}</h2><p>{comfyMessages.settingsDescription}</p></div><div className="settings-controls">
        <label>{comfyMessages.serverAddress}<input value={settings.comfyBaseUrl} placeholder="http://127.0.0.1:8188/" onChange={(event) => settings.setComfyBaseUrl(event.target.value)} /></label>
        <label>{comfyMessages.historyRecords}<input type="number" min="10" max="1000" value={settings.comfyHistoryLimit} onChange={(event) => settings.setComfyHistoryLimit(Number(event.target.value))} /></label>
        <label>{comfyMessages.storageLimit}<input type="number" min="64" max="10240" value={Math.round(settings.comfyStorageLimitBytes / 1024 ** 2)} onChange={(event) => settings.setComfyStorageLimitBytes(Number(event.target.value) * 1024 ** 2)} /></label>
        <label className="checkbox-setting"><input type="checkbox" checked={settings.comfyReplaceReverseWithTags} onChange={(event) => settings.setComfyReplaceReverseWithTags(event.target.checked)} /> {comfyMessages.replaceReverse}</label>
        <label className="checkbox-setting"><input type="checkbox" checked={settings.comfyCacheOutputs} onChange={(event) => settings.setComfyCacheOutputs(event.target.checked)} /> {comfyMessages.cacheOutputs}</label>
        <div className="settings-permission-control"><label className="checkbox-setting"><input type="checkbox" checked={settings.comfyPageIntegrationEnabled} onChange={(event) => void togglePageIntegration(event.target.checked)} /> {comfyMessages.pageIntegration}</label><p>{comfyMessages.pageIntegrationDescription}</p></div>
        <label>{comfyMessages.pageImageMinPixels}<input type="number" min="1" max="100000000" step="65536" disabled={!settings.comfyPageIntegrationEnabled} value={settings.comfyPageImageMinPixels} onChange={(event) => settings.setComfyPageImageMinPixels(Number(event.target.value))} /></label>
        {pagePermissionDenied && <p className="source-test-result" role="alert">{comfyMessages.pagePermissionDenied}</p>}
        <button type="button" className="primary-button" onClick={() => void saveComfy()}>{comfySaved ? <Check size={16} /> : <Sparkles size={16} />}{comfySaved ? comfyMessages.saved : comfyMessages.saveSettings}</button>
      </div></section>
