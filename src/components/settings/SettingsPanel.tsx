...
import { useSettingsStore, type DetailImageQuality, type Theme } from '../../stores/settings-store';
...
export function SettingsPanel() {
  ...
  return (
    <main className="settings-page">
      ...
      <section className="settings-section"><div><h2>{shellMessages.settings.appearance}</h2><p>{shellMessages.settings.appearanceDescription}</p></div><div className="settings-controls"><label>{shellMessages.settings.theme}<select value={settings.theme} onChange={(event) => settings.setTheme(event.target.value as Theme)}><option value="system">{shellMessages.settings.followSystem}</option><option value="light">{shellMessages.settings.light}</option><option value="dark">{shellMessages.settings.dark}</option></select></label><label>{shellMessages.settings.defaultColumns}<input type="number" min="2" max="8" value={settings.columns} onChange={(event) => settings.setColumns(Number(event.target.value))} /></label><label>{shellMessages.settings.detailImageQuality}<select value={settings.detailImageQuality} onChange={(event) => settings.setDetailImageQuality(event.target.value as DetailImageQuality)}><option value="preview">{shellMessages.settings.previewQuality}</option><option value="sample">{shellMessages.settings.sampleQuality}</option><option value="original">{shellMessages.settings.originalQuality}</option></select></label></div></section>
