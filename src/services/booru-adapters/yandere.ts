...
export const yandereAdapter: BooruAdapter = {
  ...
  async autocomplete(query: string, _credentials, signal): Promise<TagAutocompleteResult[]> { if (query.trim().length < 2) return []; const url = new URL('/tag.json', this.baseUrl); url.searchParams.set('name', `${query.trim()}*`); url.searchParams.set('limit', '8'); const tags = await apiGet<Array<{ name: string; count: number; type: number }>>(url, undefined, signal); return tags.map((tag) => ({ name: tag.name, label: tag.name, postCount: tag.count, category: tagCategoryFromType(tag.type) })).map((item) => { rememberTagCategory('yandere', item.name, item.category); return item; }); },
};
