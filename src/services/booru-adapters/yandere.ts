...
const normalize = (raw: YanderePost): UnifiedPost => {
  const createdAt = new Date((raw.created_at ?? 0) * 1000).toISOString();
  return { id: raw.id, source: 'yandere', rating: (['s', 'q', 'e'].includes(raw.rating) ? raw.rating : 's') as Rating, tags: raw.tags.split(/\s+/).filter(Boolean).map((name) => ({ name, category: 'general' })), tagString: raw.tags, score: raw.score ?? 0, upScore: 0, downScore: 0, favCount: 0, uploader: raw.author ?? 'unknown', sourceUrl: raw.source ?? '', imageWidth: raw.width ?? 0, imageHeight: raw.height ?? 0, fileSize: raw.file_size ?? 0, fileExt: raw.file_url?.split('.').at(-1) ?? '', previewUrl: raw.preview_url ?? '', sampleUrl: raw.sample_url ?? raw.file_url ?? '', fileUrl: raw.file_url ?? '', md5: raw.md5 ?? '', createdAt, updatedAt: createdAt, parentId: raw.parent_id ?? null, hasChildren: raw.has_children ?? false, poolIds: [], tagStringGeneral: raw.tags, tagStringArtist: '', tagStringCopyright: '', tagStringCharacter: '', tagStringMeta: '' };
};
export const yandereAdapter: BooruAdapter = {
  ...
  async searchPosts(query: SearchQuery, credentials): Promise<PaginatedResult<UnifiedPost>> { const limit = Math.min(query.limit ?? 40, 100); const page = Math.max(query.page ?? 1, 1); const url = new URL('/post.json', this.baseUrl); url.searchParams.set('limit', String(limit)); url.searchParams.set('page', String(page)); const terms = buildSourceTags('yandere', query); if (terms) url.searchParams.set('tags', terms); const posts = await apiGet<YanderePost[]>(url, credentials); const items = posts.map(normalize).filter((post) => isOnOrAfter(post.createdAt, query.dateAfter)); return { items, page, limit, hasMore: posts.length === limit }; },
